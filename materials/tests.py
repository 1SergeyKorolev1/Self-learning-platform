...
class MaterialsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.lesson = Lesson.objects.create(name='Основы программирования', course=self.course, owner=self.user)
        self.test = Test.objects.create(owner=self.user, lesson=self.lesson,
                                        correct_answer="конкатенация", description="Сложение строк?",
                                        name="Строки")
        self.attempt_answer = AttemptAnswer.objects.create(answer="конкатенация", test=self.test,
                                                           owner=self.user)
    ...
    def test_course_update(self):
        ...
        self.assertEqual(
            data_.get('name'), 'Pyton - с нуля'
        )
        self.assertEqual(
            data_.get('owner'), self.user_.pk
        )
    ...
    def test_test_retrieve(self):
        ...
        self.assertEqual(
            data.get('lesson'), self.lesson.pk
        )
        self.assertEqual(
            data.get('correct_answer'), self.test.correct_answer
        )
        self.assertEqual(
            data.get('description'), self.test.description
        )
    ...
    def test_test_update(self):
        ...
        self.assertEqual(
            data_.get('name'), 'операции со строками'
        )
        self.assertEqual(
            data_.get('owner'), self.user_.pk
        )
        self.assertEqual(
            data.get('correct_answer'), 'к-онкатенация'
        )
        self.assertEqual(
            data.get('description'), 'как называется сложение строк'
        )